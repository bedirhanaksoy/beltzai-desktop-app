...
class CustomPartTracker:
    ...
    def run(self):
        ...
        while True:
            ...
            # Run YOLO detection
            self.yolo_detections = self.model(self.frame, verbose=False)
            current_time = time.time()
            
            # Check if any object is inside the box
            self.is_right_box_empty, self.is_left_box_empty = self.check_if_box_is_empty(self.yolo_detections[0].boxes.data) 

            # Process each detection
            for det in self.yolo_detections[0].boxes.data:
                x1, y1, x2, y2, conf, cls = det 
            ...
            if key == ord('q'):
                ...
                self.crop_and_save(self.boxes[0], right_base_image_path)
            elif key == ord('2') and len(self.boxes) >= 2:
                self.crop_and_save(self.boxes[1], left_base_image_path)
            elif key == ord('c'):
                self.boxes = []
